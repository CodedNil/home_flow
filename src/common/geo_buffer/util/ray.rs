use super::{feq, fgeq, fgt, Coordinate};
use std::fmt;
...
impl Ray {
    ...
    pub fn bisector(&self, rhs: &Self, origin: Coordinate, orient: bool) -> Self {
    ...
    pub fn orientation(&self, rhs: &Coordinate) -> i32 {
        let result = self.angle.outer_product(&(*rhs - self.origin));
        if feq(result, 0.) {
            return 0;
        }
        if fgt(result, 0.) {
